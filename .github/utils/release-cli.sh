...
# FuneralAcademy CLI release script. Bumps the CLI version on dev, tags `cli-<version>`
...
if [ -z "$VERSION" ]; then
  echo ""
  echo "  📦 FuneralAcademy CLI Release Script"
  echo ""
...
echo ""
echo "  🚀 FuneralAcademy CLI Release — ${VERSION}"
echo "  ─────────────────────────────────────"
echo ""
...
PUBLISHED="$(npm view funeralacademy@"$VERSION" version 2>/dev/null || true)"
[ -n "$PUBLISHED" ] && die "funeralacademy@$VERSION is already published on npm. Pick a new version."
...
CHANGELOG_FILE=$(mktemp)
{
  ...
  echo ""
  echo "\`\`\`bash"
  echo "npx funeralacademy@${VERSION}"
  echo "\`\`\`"
  echo ""
