...
The setup wizard (`npx funeralacademy@latest setup`) is an interactive 6-step process that generates a complete Docker Compose deployment. You can go back and edit any step before confirming.

## Step 1 — Install Directory

Choose where the deployment files will be created.

- **Default**: `./funeralacademy`
...
- **HTTPS options** (non-localhost only):
  ...
  - **Manual SSL** — You handle the reverse proxy or Cloudflare. FuneralAcademy runs on HTTP internally.
...
| --- | --- | --- |
| `funeralacademy-app-{id}` | `ghcr.io/learnhouse/app:latest` (or `:dev`) | Next.js + FastAPI application |
| `funeralacademy-db-{id}` | `pgvector/pgvector:pg16` | PostgreSQL database |
| `funeralacademy-redis-{id}` | `redis:7.2.3-alpine` | Redis cache |
| `funeralacademy-caddy-{id}` | `caddy:2-alpine` | Reverse proxy with auto-SSL |
| `funeralacademy-nginx-{id}` | `nginx:alpine` | Reverse proxy (no auto-SSL) |
...
To pin to a specific image tag after setup, use `funeralacademy update --version <tag>`.
