...
Full reference for all FuneralAcademy CLI commands.
...
```bash
npx funeralacademy stop
```

## `status`

Show the live status of each FuneralAcademy container.

```bash
npx funeralacademy status
```
...
```bash
npx funeralacademy config
```

## `update`

Update FuneralAcademy to the latest release or a specific version. See [Updates](/self-hosting/maintenance/updates) for details.
...
npx funeralacademy update --to 1.4.2
# or using the short form:
npx funeralacademy update -v 1.4.2
```
...
If the migration step fails, the CLI prints the rollback command and exits with a non-zero code. Your pre-upgrade backup is in `{installDir}/backups/`. To roll back: restore the backup, revert the image tag in `docker-compose.yml`, and run `npx funeralacademy start`.
</Callout>
...
Backups are saved to `{installDir}/backups/funeralacademy-backup-{timestamp}.tar.gz`.
...
```bash
npx funeralacademy restore <archive.tar.gz>
```

Alternatively:

```bash
npx funeralacademy backup --restore <archive.tar.gz>
```

<Callout type="warning">
Restore only works with local (Docker) databases, not external Postgres connections. Restart services after restoring: `npx funeralacademy stop && npx funeralacademy start`.
</Callout>
...
```bash
npx funeralacademy deployments
```

Features:
- List all FuneralAcademy deployments with container status
- Scale memory limits for `funeralacademy-app`, `db`, and `redis`
...
Run `npx funeralacademy doctor` — it scans logs and surfaces the root cause. Common causes:
...
If `npx funeralacademy update` reports a migration failure:

```bash
# 1. Restore your pre-upgrade backup
npx funeralacademy restore backups/db-pre-upgrade-*.sql.gz
...
Then check the [FuneralAcademy releases page](https://github.com/learnhouse/learnhouse/releases) and the [GitHub issues](https://github.com/learnhouse/learnhouse/issues) for your version before retrying.

### Database unreachable after update

The `funeralacademy-db` container may need a moment after startup. Wait 20-30 seconds and run `npx funeralacademy health`. If health checks keep failing, run `npx funeralacademy logs` to see the Postgres logs.
...
docker compose exec db psql -U funeralacademy -c "DROP DATABASE funeralacademy; CREATE DATABASE funeralacademy;"
npx funeralacademy restore <backup.tar.gz>
```
