...
- **Infrastructure** (Docker): PostgreSQL with pgvector + Redis run in containers via `.funeralacademy/docker-compose.dev.yml`
...
```bash
docker compose -f .funeralacademy/docker-compose.dev.yml -p funeralacademy-dev down
```
