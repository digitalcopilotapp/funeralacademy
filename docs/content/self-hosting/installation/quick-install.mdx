...
Follow these steps to install and start a self-hosted FuneralAcademy instance using the FuneralAcademy CLI.
...
The wizard generates `docker-compose.yml`, `.env`, `funeralacademy.config.json`, and an nginx or Caddy configuration based on your choices. Sensible defaults are provided for most options -- you can press Enter to accept them and adjust settings later.

## Step 2: Start FuneralAcademy
...
## Step 3: Access FuneralAcademy

Once all services are running, access FuneralAcademy at your configured domain or:
