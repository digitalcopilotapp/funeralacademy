...
Monitoring your self-hosted FuneralAcademy instance helps you catch issues early and ensure a reliable experience for your users.
...
```bash
funeralacademy status    # quick container status for each service
funeralacademy health    # PostgreSQL, Redis, HTTP health endpoint, disk, resources
funeralacademy doctor    # deep diagnostics (Docker, ports, DNS, env validation, logs)
```
...
- Run `funeralacademy doctor` regularly to verify all services are running and healthy
...
- FuneralAcademy requires at least 2 GB of RAM
...
- **Uptime monitoring** -- use an external service to check that your FuneralAcademy instance is accessible (e.g., monitor `http://your-domain/api/v1/health`)
