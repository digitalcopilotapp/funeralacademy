...
FuneralAcademy supports two storage backends for uploaded content such as images, videos, and documents.
...
Content is persisted in a Docker volume, so it survives container restarts. The `funeralacademy backup` command includes filesystem content in its backups.
...
FUNERALACADEMY_S3_API_BUCKET_NAME=funeralacademy-content
FUNERALACADEMY_S3_API_ENDPOINT_URL=https://s3.amazonaws.com
```
...
| Backup | Included in `funeralacademy backup` | Managed separately by your storage provider |
...
If you want S3-compatible storage without relying on a cloud provider, you can run MinIO alongside FuneralAcademy. MinIO provides an S3-compatible API on your own infrastructure.
...
FUNERALACADEMY_CONTENT_DELIVERY_TYPE=s3api
FUNERALACADEMY_S3_API_BUCKET_NAME=funeralacademy-content
FUNERALACADEMY_S3_API_ENDPOINT_URL=http://minio:9000
```
