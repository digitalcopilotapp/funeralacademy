...
FuneralAcademy supports two tenancy modes, selected by a single environment variable: `FUNERALACADEMY_TENANCY`.
...
```
FUNERALACADEMY_TENANCY=multi   # requires EE (or SaaS) + FUNERALACADEMY_DOMAIN
FUNERALACADEMY_TENANCY=single  # default for OSS / dev / VPS self-host
# unset                    # inferred — see "Default inference" below
```

The backend validates at boot:
- `multi` requires `is_ee_available()` OR `FUNERALACADEMY_SAAS=true`, AND `FUNERALACADEMY_DOMAIN` set to a routable parent domain (not `localhost`). Mis-configuration aborts startup with a clear error.
- `single` ignores `FUNERALACADEMY_DOMAIN` and `FUNERALACADEMY_COOKIE_DOMAIN`. If you set them, you'll see a deprecation warning at boot — they are no longer authoritative.

### Default inference (when `FUNERALACADEMY_TENANCY` is unset)
...
|----------------------------------------------------------------------------------------------|------------------|
| `FUNERALACADEMY_SAAS=true` + real (non-localhost) domain + not self-hosted/dev                   | `multi`          |
| EE folder present + `FUNERALACADEMY_COOKIE_DOMAIN` starts with `.` + real domain + not self/dev  | `multi`          |
| Anything else                                                                                | `single`         |

The strong signal for "I want subdomain tenancy" is a dotted cookie domain (`FUNERALACADEMY_COOKIE_DOMAIN=.foo.com`) — operators only set that when they want subdomains to share auth. This means an EE deployment on a single VPS domain (no dotted cookie domain) defaults to `single`, which is the right behavior for the self-host-EE case (one org, EE features locally available).

**Recommended:** set `FUNERALACADEMY_TENANCY` explicitly in production. Inference is a backward-compat convenience, not a long-term contract.

## Compatibility with deployment modes (oss / ee / saas)

`FUNERALACADEMY_TENANCY` is orthogonal to the existing **deployment mode** (`oss`, `ee`, `saas`) which is set by the presence of the `ee/` folder and `FUNERALACADEMY_SAAS`. The matrix:
...
Existing EE/SaaS deployments that haven't set `FUNERALACADEMY_TENANCY` continue to behave as before because the inference reads their existing config (`FUNERALACADEMY_SAAS`, `FUNERALACADEMY_COOKIE_DOMAIN`, `FUNERALACADEMY_DOMAIN`) and picks `multi` when those signal subdomain intent.

## Mode 1 — Multi (EE)

Required env:
- `FUNERALACADEMY_TENANCY=multi`
- `FUNERALACADEMY_DOMAIN=your-domain.tld` (the apex; e.g. `funeralacademy.io`)
- `FUNERALACADEMY_COOKIE_DOMAIN=.your-domain.tld` (so subdomains share the session)
- `ee/` folder present (or unset `FUNERALACADEMY_DISABLE_EE`)
...
Required env:
- `FUNERALACADEMY_TENANCY=single` (or just leave unset; this is the default)
...
All FuneralAcademy cookies use the `LH_` prefix.
...
The cookie naming convention changed in this release. **All cookies now use the `LH_` prefix and legacy names are no longer read.** Old cookie names (`access_token_cookie`, `refresh_token_cookie`, `funeralacademy_orgslug`, `funeralacademy_tenancy`, `funeralacademy_multi_org`, `funeralacademy_current_orgslug`, `funeralacademy_has_session`, `lh_region`, etc.) are dropped entirely — there is no read-fallback window.
...
|------------|----------------------------------------------------------------------------|----------------------------------------------|
| `multi`    | Configured via `FUNERALACADEMY_ALLOWED_REGEXP` (matches apex + subdomains)     | Strict allowlist for known multi-tenant domain |
...
```json
{
  ...
  "frontend_domain": "funeralacademy.io",
  "top_domain": "funeralacademy.io"
}
```
...
`_is_allowed_base_url` validates the URL against an allowlist before returning it. In `single` mode the operator's host is authoritative — any well-formed http(s) URL is accepted. In `multi` mode, the URL must match `FUNERALACADEMY_ALLOWED_ORIGINS` / `FUNERALACADEMY_ALLOWED_REGEXP` / `FUNERALACADEMY_PLATFORM_URL`.

## EE / OSS code split

The OSS frontend never imports subdomain or custom-domain logic directly. All of that lives under `funeralacademy/apps/web/ee/services/tenancy/`:
...
The OSS `proxy.ts` and `services/org/orgResolution.ts` reach these via dynamic `import()` wrapped in try/catch. If EE is unavailable (folder removed at deploy time, or `FUNERALACADEMY_DISABLE_EE=1`) the resolver falls back to the default org and logs a warning.
...
```sh
FUNERALACADEMY_TENANCY=multi
FUNERALACADEMY_DOMAIN=lh.test
FUNERALACADEMY_COOKIE_DOMAIN=.lh.test
```
Add hosts entries:
```
...
```sh
FUNERALACADEMY_TENANCY=single
# FUNERALACADEMY_DOMAIN unset
```
...
```sh
FUNERALACADEMY_DISABLE_EE=1
FUNERALACADEMY_TENANCY=multi   # should fail at boot
FUNERALACADEMY_TENANCY=single  # should boot cleanly
```
With `single`, no EE imports are attempted at runtime.

## Upgrading from versions before `FUNERALACADEMY_TENANCY` existed

Prior versions decided multi-vs-single behavior implicitly from `is_multi_org_allowed()` (deployment mode in `ee`/`saas`), `self_hosted`, `development_mode`, and `FUNERALACADEMY_USE_DEFAULT_ORG`. After the introduction of `FUNERALACADEMY_TENANCY` these flags are deprecated and the new env var is the source of truth.
...
1. Check the boot log for any deprecation warnings about `FUNERALACADEMY_USE_DEFAULT_ORG`, `FUNERALACADEMY_SELF_HOSTED`, or `FUNERALACADEMY_COOKIE_DOMAIN` being set in single mode.
2. Hit `GET /api/v1/instance/info` and confirm `tenancy` matches your expectation.
3. Set `FUNERALACADEMY_TENANCY` explicitly in your env config to lock the behavior — inference is a backward-compat convenience, not a long-term contract.
...
2. **`multi` mode** requires:
   - EE folder included in the build (`docker build --build-arg FUNERALACADEMY_PUBLIC=false`)
   ...
   - Backend env: `FUNERALACADEMY_TENANCY=multi`, `FUNERALACADEMY_DOMAIN`, `FUNERALACADEMY_COOKIE_DOMAIN`, `FUNERALACADEMY_ALLOWED_REGEXP`
3. **`single` mode** on a VPS:
   - Point any domain at the server. No env config beyond `FUNERALACADEMY_TENANCY=single` is needed.
...
Backend:
- `funeralacademy/apps/api/config/config.py` — `FUNERALACADEMY_TENANCY` env, validation
- `funeralacademy/apps/api/src/routers/instance.py` — `instance/info` endpoint
- `funeralacademy/apps/api/src/routers/auth.py` — `get_cookie_domain_for_request`
- `funeralacademy/apps/api/src/services/email/utils.py` — `get_org_signup_base_url`
- `funeralacademy/apps/api/app.py` — CORS branching
- `funeralacademy/apps/api/src/core/ee_hooks.py` — `is_multi_org_allowed`, EE detection

Frontend:
- `funeralacademy/apps/web/proxy.ts` — middleware (resolveTenant)
- `funeralacademy/apps/web/ee/services/tenancy/core.ts` — pure resolver logic
- `funeralacademy/apps/web/ee/services/tenancy/resolveMulti.middleware.ts` — Edge entry
- `funeralacademy/apps/web/ee/services/tenancy/resolveMulti.server.ts` — Node entry
- `funeralacademy/apps/web/services/org/orgResolution.ts` — server-side org resolution
- `funeralacademy/apps/web/services/auth/cookies.ts` — cookie options
- `funeralacademy/apps/web/services/config/config.ts` — `getTenancy`, `getUriWithOrg`
