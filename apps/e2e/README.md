# FuneralAcademy E2E — UI acceptance tests

Browser-driven, human-like end-to-end tests for FuneralAcademy. The suite boots a
real **self-host via the FuneralAcademy CLI** (`setup --ci`, pulling the published
...
| `E2E_CLI` | `npx --yes funeralacademy@latest` | CLI used to boot (override for a local build). |
...
> instance (it passes against `funeralacademy dev` / a branch-built image).
