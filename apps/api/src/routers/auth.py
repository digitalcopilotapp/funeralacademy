...
def get_cookie_domain_for_request(request: Request) -> str | None:
    ...
    - tenancy == "multi":
        - request from a subdomain of FUNERALACADEMY_DOMAIN → configured cookie
          domain (e.g. ".funeralacademy.io") so subdomains share the session.
...
_refresh_logger = logging.getLogger("funeralacademy.auth.refresh")
...
async def third_party_login(
    ...
    if org_id is not None:
        ...
        if join_mechanism == "inviteOnly":
            ...
            if not _authorized:
                _r = None
                try:
                    _lh_config = get_funeralacademy_config()
    ...
    if _consume_invite_key:
        _r = None
        try:
            _redis_url = get_funeralacademy_config().redis_config.redis_connection_string
            if _redis_url:
                _r = _redis.Redis.from_url(_redis_url)
