...
# Set the first time a request arrives with FUNERALACADEMY_PLATFORM_API_KEY unset, so
...
async def verify_platform_key(x_platform_key: str = Header(...)):
    ...
    expected_key = os.getenv("FUNERALACADEMY_PLATFORM_API_KEY", "")
    if not expected_key and not _REPORTED_MISSING_PLATFORM_KEY:
        _REPORTED_MISSING_PLATFORM_KEY = True
        logger.error(
            "FUNERALACADEMY_PLATFORM_API_KEY is not set on this deployment: every "
