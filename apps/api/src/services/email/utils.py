...
def _is_allowed_base_url(url: str) -> bool:
    ...
    config = get_funeralacademy_config()
    ...
    # The platform domain and any org subdomain of it (slug.funeralacademy.io).
    if req_host in configured_hosts:
        return True
    ...
    # Check against FUNERALACADEMY_PLATFORM_URL (the main platform, e.g. https://www.funeralacademy.app)
    platform_url = os.environ.get("FUNERALACADEMY_PLATFORM_URL", "").rstrip("/")
    if platform_url:
        platform_host = (
...
def _configured_frontend_base_url() -> Optional[str]:
    ...
    """
    platform_url = os.environ.get("FUNERALACADEMY_PLATFORM_URL")
    if platform_url:
...
def get_media_base_url(request: Optional[Request] = None) -> str:
    ...
    (``NEXT_PUBLIC_FUNERALACADEMY_BACKEND_URL`` / ``..._MEDIA_URL``). The backend
    doesn't have that value, so resolve it in order:

    1. An explicit backend override (``FUNERALACADEMY_MEDIA_URL`` /
       ``FUNERALACADEMY_BACKEND_URL``) — set this if the media host isn't ``api.``.
    2. The SaaS convention ``{scheme}://api.{domain}`` (e.g. api.funeralacademy.io)
    ...
    """
    override = os.environ.get("FUNERALACADEMY_MEDIA_URL") or os.environ.get(
...
def get_org_logo_url(org, request: Optional[Request] = None) -> Optional[str]:
    ...
    to the default Funeral Academy mark.
    """
    ...
    if not base:
        ...
        # back to the default Funeral Academy mark instead.
        return None
...
def send_email(
    ...
    sender = f"Funeral Academy <{mailing.system_email_address}>"
