...
def _unsub_secret() -> bytes:
    ...
    """
    secret = get_funeralacademy_config().security_config.auth_jwt_secret_key
    return hashlib.sha256(f"funeralacademy.unsubscribe.{_TOKEN_VERSION}|".encode() + secret.encode()).digest()
