...
def nudges_enabled() -> bool:
    return _flag("FUNERALACADEMY_NUDGES_ENABLED", False)
...
def backfill_mode() -> str:
    ...
    """
    return (os.environ.get("FUNERALACADEMY_NUDGES_BACKFILL_MODE") or "off").strip().lower()
...
async def activation_date(
    ...
    tracks. An explicit `FUNERALACADEMY_NUDGES_ACTIVATION_DATE` wins; otherwise it
    ...
    """
    raw = (os.environ.get("FUNERALACADEMY_NUDGES_ACTIVATION_DATE") or "").strip()
    if raw:
        try:
            ...
            logger.warning(
                "Ignoring unparseable FUNERALACADEMY_NUDGES_ACTIVATION_DATE: %r", raw
            )
...
async def run_nudges(
    ...
    *,
    ...
    if not force and not dry_run and not seed and not nudges_enabled():
        logger.info("Nudges skipped: FUNERALACADEMY_NUDGES_ENABLED is not set")
        return stats
