...
def _lock_key(day: datetime) -> str:
    return f"funeralacademy:nudges:daily:{day.date().isoformat()}"
...
def start_scheduler() -> None:
    ...
    try:
        ...
        if not nudges_enabled():
            logger.info(
                "Nudge scheduler idle: FUNERALACADEMY_NUDGES_ENABLED is not set"
            )
            return
        ...
        if mode != "saas":
            ...
            return
        if os.environ.get("FUNERALACADEMY_NUDGES_NO_SCHEDULER"):
