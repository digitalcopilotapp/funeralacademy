...
# Funeralacademy domain for CNAME instructions
FUNERALACADEMY_DOMAIN = os.getenv('FUNERALACADEMY_DOMAIN', 'funeralacademy.io')
...
def is_own_agency_subdomain(domain: str, org_slug: str) -> bool:
    ...
    In agency (multi-tenant) deployments the whole `*.{FUNERALACADEMY_DOMAIN}` space
    ...
    """
    base = (FUNERALACADEMY_DOMAIN or "").lower().strip().split(":")[0]
    if not base or base in ("funeralacademy.io", "localhost"):
        return False
...
def get_verification_instructions(domain: str, token: str, org_slug: str) -> CustomDomainVerificationInfo:
    ...
    if subdomain:
        txt_record_host = f"_funeralacademy-verification.{subdomain}"
        cname_record_host = subdomain
    else:
        txt_record_host = "_funeralacademy-verification"
        cname_record_host = "@"

    txt_record_value = f"funeralacademy-verify={token}"
    cname_record_value = f"{org_slug}.{FUNERALACADEMY_DOMAIN}"
...
async def verify_domain_dns(domain: CustomDomain, db_session: AsyncSession, org_slug: str) -> Tuple[bool, str]:
    ...
    if os.getenv('FUNERALACADEMY_CUSTOM_DOMAIN_DEV_MODE') == 'true':
    ...
    if is_own_agency_subdomain(domain.domain, org_slug):
        logger.info(f"Auto-verifying {domain.domain} (own slug host under agency domain {FUNERALACADEMY_DOMAIN})")
        domain.status = "verified"
        domain.verified_at = str(datetime.now())
        domain.last_check_at = str(datetime.now())
        domain.check_error = None
    ...
    try:
        ...
        # Check TXT record
        txt_host = f"_funeralacademy-verification.{domain.domain}"
        expected_txt = f"funeralacademy-verify={domain.verification_token}"
...
async def add_custom_domain(
    ...
    if auto_verified:
        logger.info(f"Auto-verified {domain} on add (subdomain of agency domain {FUNERALACADEMY_DOMAIN})")
