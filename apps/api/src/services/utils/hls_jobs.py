...
- An in-process consumer (`FUNERALACADEMY_HLS_INPROCESS_WORKER=true`, default off,
  Semaphore(1)) for dev/self-host.

Everything is gated by `FUNERALACADEMY_HLS_ENABLED` (default off). Until a video is
...
REDIS_QUEUE_KEY = "funeralacademy:hls:queue"
...
def hls_max_retries() -> int:
    ...
    (FUNERALACADEMY_HLS_MAX_RETRIES, default 6)."""
    try:
        return max(1, int(os.environ.get("FUNERALACADEMY_HLS_MAX_RETRIES", "6")))
    except (TypeError, ValueError):
        return 6


def _lease_key(activity_uuid: str) -> str:
    return f"funeralacademy:hls:lease:{activity_uuid}"


def _retries_key(activity_uuid: str) -> str:
    return f"funeralacademy:hls:retries:{activity_uuid}"
...
def hls_enabled() -> bool:
    return _flag("FUNERALACADEMY_HLS_ENABLED")


def hls_concurrency() -> int:
    """Max concurrent transcodes per API pod (FUNERALACADEMY_HLS_CONCURRENCY, default 1)."""
    try:
        return max(1, int(os.environ.get("FUNERALACADEMY_HLS_CONCURRENCY", "1")))
    except (TypeError, ValueError):
        return 1
