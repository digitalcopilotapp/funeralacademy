...
def encryption_enabled() -> bool:
    return os.environ.get("FUNERALACADEMY_HLS_ENCRYPT", "true").strip().lower() == "true"
...
def ffmpeg_threads() -> str:
    """ffmpeg `-threads` value (FUNERALACADEMY_HLS_FFMPEG_THREADS, default '1').

    Kept as a string for the arg list; sanitized to a small non-negative int
    ('0' means ffmpeg auto = all cores)."""
    raw = os.environ.get("FUNERALACADEMY_HLS_FFMPEG_THREADS", "1")
    try:
...
def build_ffmpeg_args(
    ...
    # FUNERALACADEMY_HLS_FFMPEG_THREADS if you have headroom and want faster encodes.
