...
The encryption key is derived via HKDF from ``FUNERALACADEMY_MFA_ENCRYPTION_KEY``
...
zero-config for existing deployments, but it couples the two:
    ...
    ⚠️  Rotating the JWT secret without setting FUNERALACADEMY_MFA_ENCRYPTION_KEY
...
_MFA_KEY_INFO = b"funeralacademy-mfa-secret-encryption-v1"
...
def _get_fernet() -> Fernet:
    ...
    material = os.environ.get("FUNERALACADEMY_MFA_ENCRYPTION_KEY") or SECRET_KEY
    if not material:  # pragma: no cover - SECRET_KEY is always configured in a running app
        raise RuntimeError(
            ...
            "FUNERALACADEMY_MFA_ENCRYPTION_KEY or the JWT secret."
        )
