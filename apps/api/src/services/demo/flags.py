...
def demo_enabled() -> bool:
    return _flag("FUNERALACADEMY_DEMO_ENABLED", False)


def demo_slug() -> str:
    raw = (os.environ.get("FUNERALACADEMY_DEMO_SLUG") or "").strip().lower()
    return raw or DEFAULT_SLUG


def refresh_minutes() -> int:
    return _int_flag("FUNERALACADEMY_DEMO_REFRESH_MINUTES", DEFAULT_REFRESH_MINUTES)


def scheduler_disabled() -> bool:
    """For deployments that would rather drive ``demo-refresh`` from their own cron."""
    return _flag("FUNERALACADEMY_DEMO_NO_SCHEDULER", False)
