...
# Inline SVG logo (FuneralAcademy icon mark + wordmark, scaled down)
...
def _org_logo_img(logo_url: str, alt: str) -> str:
    ...
    Bounded to the same footprint as the FuneralAcademy wordmark. Raster logos
    ...
    org-branded, never a broken FuneralAcademy mark.
    """
    return (
...
def _email_layout(
    ...
    ``logo_html`` defaults to the FuneralAcademy mark; white-labeled emails pass the
...
def send_account_creation_email(
    ...
    the subject and body name the org (not FuneralAcademy), the org's ``logo_url``
    replaces the FuneralAcademy mark when present, and the footer is reduced to a
    subtle "Powered by FuneralAcademy". Org-less signups keep the FuneralAcademy-branded
...
def send_org_join_email(
    ...
    academy, not onto FuneralAcademy — with the org's logo when it has one.
    """
