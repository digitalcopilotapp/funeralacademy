...
ACADEMY_URL = "https://university.funeralacademy.io"


# Inline SVG logo (Funeral Academy icon mark + wordmark, scaled down)
...
def _org_logo_img(logo_url: str, alt: str) -> str:
    ...
    Bounded to the same footprint as the Funeral Academy wordmark. Raster logos
    ...
    org-branded, never a broken Funeral Academy mark.
    """
    return (
...
def _reply_to_address() -> str:
    ...
    try:
        ...
        return (get_funeralacademy_config().contact_email or "").strip()
...
def _email_layout(
    ...
    ``logo_html`` defaults to the Funeral Academy mark; white-labeled emails pass the
...
def send_account_creation_email(
    ...
    the subject and body name the org (not Funeral Academy), the org's ``logo_url``
    replaces the Funeral Academy mark when present, and the footer is reduced to a
    subtle "Powered by Funeral Academy". Org-less signups keep the Funeral Academy-branded
...
def send_org_join_email(
    ...
    academy, not onto Funeral Academy — with the org's logo when it has one.
    """
