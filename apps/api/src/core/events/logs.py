...
def _log_level() -> int:
    raw = (os.environ.get("FUNERALACADEMY_LOG_LEVEL") or "INFO").strip().upper()
...
def init_logging() -> None:
    ...
    No file handler. In a container nothing reads ``logs/funeralacademy.log`` and
    ...
    already = any(
        isinstance(h, logging.StreamHandler) and getattr(h, "_funeralacademy", False)
    ...
    handler._funeralacademy = True  # type: ignore[attr-defined]
