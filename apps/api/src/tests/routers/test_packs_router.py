...
class TestInternalPacksRouter:
    async def test_verify_platform_key_errors(self, client):
        old = os.environ.get("FUNERALACADEMY_PLATFORM_API_KEY")
        os.environ.pop("FUNERALACADEMY_PLATFORM_API_KEY", None)
        try:
            ...
            if old is None:
                ...
                os.environ["FUNERALACADEMY_PLATFORM_API_KEY"] = old

        assert response.status_code == 403

        old = os.environ.get("FUNERALACADEMY_PLATFORM_API_KEY")
        os.environ["FUNERALACADEMY_PLATFORM_API_KEY"] = "platform-secret"
        try:
            response = await client.post(
                "/api/v1/internal/packs/1/activate",
