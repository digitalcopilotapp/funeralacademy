...
def _mock_config(tenancy: str = "multi"):
    return SimpleNamespace(
        hosting_config=SimpleNamespace(
            tenancy=tenancy,
            domain="funeralacademy.test",
            cookie_config=SimpleNamespace(domain=".funeralacademy.test"),
        )
    )
...
class TestAuthHelpers:
    def test_get_cookie_domain_for_configured_subdomain(self):
        request = Mock()
        request.headers = {
            "origin": "https://app.funeralacademy.test",
            "referer": "",
            "host": "",
        }

        with patch("src.routers.auth.get_funeralacademy_config", return_value=_mock_config()):
            assert get_cookie_domain_for_request(request) == ".funeralacademy.test"
    ...
    def test_get_cookie_domain_single_tenancy_is_always_host_only(self):
        ...
        for origin in (
            "https://app.funeralacademy.test",  # would be subdomain match in multi
            ...
            with patch(
                "src.routers.auth.get_funeralacademy_config",
    ...
    def test_set_and_unset_auth_cookies(self):
        ...
        request = Mock()
        request.client = SimpleNamespace(host="127.0.0.1")
        request.headers = {"origin": "https://app.funeralacademy.test"}
    ...
    def test_cookie_domain_and_secure_fallback_branches(self):
        ...
        with patch("src.routers.auth.get_funeralacademy_config", return_value=_mock_config()), patch(
            "src.routers.auth.isDevModeEnabled",
            return_value=False,
        ):
            assert get_cookie_domain_for_request(empty_request) == ".funeralacademy.test"
...
class TestAuthRouter:
    ...
    async def test_oauth_invalid_org_id_returns_400(self, client, db, org):
        with patch("src.routers.auth.get_funeralacademy_config"):
            response = await client.post(
                "/api/v1/auth/oauth",
                params={"org_id": 9999},
                json={
    ...
    async def test_oauth_invite_only_org_without_invite_returns_403(self, client, db, org):
        ...
        with patch("src.routers.auth.get_funeralacademy_config", return_value=mock_config), patch(
    ...
    async def test_oauth_org_id_google_verified_checks_invite_and_closes_redis(
        ...
        with patch(
            ...
            "src.routers.auth.get_funeralacademy_config", return_value=mock_config
        ), patch(
            "redis.Redis.from_url", return_value=mock_redis
...
class TestRefreshOutcomeTelemetry:
    ...
    async def test_missing_cookie_logs_cookie_missing(self, client, caplog):
        with patch(
            ...
            with caplog.at_level(logging.INFO, logger="funeralacademy.auth.refresh"):
                response = await client.get("/api/v1/auth/refresh")
        assert response.status_code == 401
