...
def test_init_logging_attaches_a_stdout_handler():
    ...
    try:
        ...
        ours = [h for h in root.handlers if getattr(h, "_funeralacademy", False)]
        assert len(ours) == 1
...
def test_init_logging_is_idempotent():
    ...
    try:
        ...
        logs.init_logging()
        logs.init_logging()
        assert len([h for h in root.handlers if getattr(h, "_funeralacademy", False)]) == 1
    finally:
        root.handlers = before


def test_init_logging_honours_an_explicit_level(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_LOG_LEVEL", "WARNING")
    root = logging.getLogger()
...
def test_is_ee_available(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_DISABLE_EE", "1")
    assert ee_hooks.is_ee_available() is False

    monkeypatch.delenv("FUNERALACADEMY_DISABLE_EE", raising=False)
...
async def test_startup_and_shutdown_app(monkeypatch):
    ...
    monkeypatch.setattr(events, "get_funeralacademy_config", lambda: SimpleNamespace(name="cfg"))
    ...
    assert app.funeralacademy_config.name == "cfg"
