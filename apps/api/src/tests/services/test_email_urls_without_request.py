...
def _config(
    tenancy="multi", domain="funeralacademy.io", frontend_domain="", ssl=True
):
...
def clean_env(monkeypatch):
    for var in (
        "FUNERALACADEMY_PLATFORM_URL",
        "FUNERALACADEMY_MEDIA_URL",
        "FUNERALACADEMY_BACKEND_URL",
    ):
...
class TestConfiguredFrontendBaseUrl:
    def test_platform_url_wins(self, monkeypatch, clean_env):
        monkeypatch.setenv("FUNERALACADEMY_PLATFORM_URL", "https://app.example.com/")
        monkeypatch.setattr(email_utils, "get_funeralacademy_config", _config)
    ...
    def test_falls_back_to_base_domain(self, monkeypatch, clean_env):
        monkeypatch.setattr(
            email_utils, "get_funeralacademy_config", lambda: _config(domain="example.com")
        )
...
class TestOrgSignupBaseUrlWithoutRequest:
    async def test_multi_tenant_builds_the_org_subdomain(self, monkeypatch, clean_env):
        ...
        assert url == "https://acme.funeralacademy.io"

    async def test_single_tenant_uses_configured_url(self, monkeypatch, clean_env):
        monkeypatch.setenv("FUNERALACADEMY_PLATFORM_URL", "https://school.example.com")
        monkeypatch.setattr(
            email_utils, "get_funeralacademy_config", lambda: _config(tenancy="single")
        )
...
class TestMediaBaseUrlWithoutRequest:
    def test_explicit_override_wins(self, monkeypatch, clean_env):
        monkeypatch.setenv("FUNERALACADEMY_MEDIA_URL", "https://media.example.com/")
    ...
    def test_backend_url_is_also_honoured(self, monkeypatch, clean_env):
        monkeypatch.setenv("FUNERALACADEMY_BACKEND_URL", "https://api.example.com")
    ...
    def test_saas_convention_is_derived_from_the_domain(self, monkeypatch, clean_env):
        monkeypatch.setattr(email_utils, "get_funeralacademy_config", _config)
        assert email_utils.get_media_base_url(None) == "https://api.funeralacademy.io"
...
class TestOrgLogoUrlWithoutRequest:
    ...
    def test_builds_an_absolute_url(self, monkeypatch, clean_env):
        ...
        assert email_utils.get_org_logo_url(self.ORG, None) == (
            "https://api.funeralacademy.io/content/orgs/org_abc/logos/logo.png"
        )
