...
def test_resolve_tts_config_no_key_raises():
    with patch.object(gen, "get_funeralacademy_config", return_value=_cfg(api_key=None, gemini_api_key=None)):
        with pytest.raises(AINotConfiguredError):
            gen._resolve_tts_config()


def test_resolve_tts_config_default_and_override():
    with patch.object(gen, "get_funeralacademy_config", return_value=_cfg(provider="google", api_key="k")):
        key, model = gen._resolve_tts_config()
        assert key == "k"
        assert model == gen.DEFAULT_TTS_MODEL
    with patch.object(
        gen, "get_funeralacademy_config",
...
async def test_generate_speech_single_voice():
    ...
    with patch.object(gen, "get_funeralacademy_config", return_value=_cfg()), \
