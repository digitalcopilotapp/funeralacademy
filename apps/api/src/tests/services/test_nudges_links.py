...
BASE = "https://acme.funeralacademy.io"
...
class TestUnsubscribeUrl:
    def test_token_round_trips_from_the_built_url(self):
        url = unsubscribe_url("https://api.funeralacademy.io", "user_abc")
        token = url.split("token=", 1)[1]
        assert verify_unsubscribe_token(token) == "user_abc"

    def test_points_at_the_api_endpoint(self):
        url = unsubscribe_url("https://api.funeralacademy.io", "user_abc")
        assert url.startswith("https://api.funeralacademy.io/api/v1/emails/unsubscribe?token=")
