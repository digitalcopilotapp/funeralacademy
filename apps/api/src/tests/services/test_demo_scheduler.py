...
def demo_on(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_DEMO_ENABLED", "1")
    monkeypatch.delenv("FUNERALACADEMY_DEMO_NO_SCHEDULER", raising=False)
...
async def test_start_does_nothing_when_the_demo_is_off(monkeypatch):
    ...
    monkeypatch.delenv("FUNERALACADEMY_DEMO_ENABLED", raising=False)
...
async def test_start_does_nothing_when_the_scheduler_is_disabled(monkeypatch, demo_on):
    ...
    """
    monkeypatch.setenv("FUNERALACADEMY_DEMO_NO_SCHEDULER", "1")
