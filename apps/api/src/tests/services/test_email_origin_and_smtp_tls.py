...
def _config(**overrides):
    hosting = SimpleNamespace(
        ...
        domain=overrides.pop("domain", "funeralacademy.io"),
        frontend_domain=overrides.pop("frontend_domain", "app.funeralacademy.io"),
...
class TestOriginAllowlist:
    def test_catch_all_regexp_no_longer_trusts_an_attacker_origin(self):
        ...
        with _patch_config(allowed_regexp=CATCH_ALL_REGEXP), _no_platform_url():
            ...
            assert not _is_allowed_base_url("https://funeralacademy.io.evil.com")
            assert not _is_allowed_base_url("https://evil.com/funeralacademy.io")

    def test_platform_domain_and_org_subdomains_are_accepted(self):
        with _patch_config(allowed_regexp=CATCH_ALL_REGEXP), _no_platform_url():
            assert _is_allowed_base_url("https://funeralacademy.io")
            assert _is_allowed_base_url("https://www.funeralacademy.io")
            assert _is_allowed_base_url("https://acme.funeralacademy.io")
            assert _is_allowed_base_url("https://app.funeralacademy.io")
            assert _is_allowed_base_url("https://funeralacademy.io/")
...
class TestScopedOriginRegexp:
    ...
    @pytest.mark.parametrize(
        ...
        [
            r"^https://scoped\.test$",
            r"^https://(?:[a-z0-9-]+\.)*funeralacademy\.io$",
        ],
    )
...
class TestMagicLinkUrlInEmailBody:
    ...
    async def test_attacker_origin_does_not_reach_the_emailed_link(
        ...
        assert "https://app.funeralacademy.io/auth/magic?token=tok" in sent["body"]
    ...
    async def test_legitimate_org_subdomain_origin_is_preserved(
        ...
        response, sent = await self._request_link(
            magic_client, link_user.email, {"origin": "https://acme.funeralacademy.io"}
        )
        assert response.status_code == 200
        assert "https://acme.funeralacademy.io/auth/magic?token=tok" in sent["body"]
