...
def _saas_and_enabled(monkeypatch):
    ...
    monkeypatch.setenv("FUNERALACADEMY_NUDGES_ENABLED", "true")
...
class TestGates:
    ...
    async def test_kill_switch_off_sends_nothing(
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_ENABLED", "false")
...
class TestBackfillGuard:
    ...
    async def test_backfill_off_excludes_preexisting_orgs(
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_BACKFILL_MODE", "off")
    ...
    async def test_winback_mode_admits_the_winback_tracks(
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_BACKFILL_MODE", "winback")
    ...
    async def test_full_mode_admits_everything(self, db, legacy_org, sender, monkeypatch):
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_BACKFILL_MODE", "full")
...
class TestEnvHelpers:
    ...
    def test_flag_parsing(self, monkeypatch, value, expected):
        monkeypatch.setenv("FUNERALACADEMY_TEST_FLAG", value)
        assert runner_module._flag("FUNERALACADEMY_TEST_FLAG") is expected

    def test_flag_default_when_unset(self, monkeypatch):
        monkeypatch.delenv("FUNERALACADEMY_TEST_FLAG", raising=False)
        assert runner_module._flag("FUNERALACADEMY_TEST_FLAG", True) is True
...
class TestExplicitActivationDate:
    ...
    async def test_an_explicit_date_wins(self, db, monkeypatch):
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_ACTIVATION_DATE", "2020-01-01")
    ...
    async def test_a_past_date_makes_existing_orgs_ordinary(
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_ACTIVATION_DATE", "2000-01-01")
    ...
    async def test_garbage_falls_back_to_the_ledger(self, db, monkeypatch):
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_ACTIVATION_DATE", "not-a-date")
    ...
    async def test_dry_run_rows_do_not_pin_the_boundary(self, db, monkeypatch):
        ...
        monkeypatch.delenv("FUNERALACADEMY_NUDGES_ACTIVATION_DATE", raising=False)
        await db.execute(delete(NudgeSend))
        db.add(
            NudgeSend(
                nudge_id="x.y",
