...
def test_flags_default_off(monkeypatch):
    monkeypatch.delenv("FUNERALACADEMY_HLS_ENABLED", raising=False)
    monkeypatch.delenv("FUNERALACADEMY_HLS_CONCURRENCY", raising=False)
    assert hls_jobs.hls_enabled() is False
    assert hls_jobs.hls_concurrency() == 1  # default


def test_flags_parse_true(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_HLS_ENABLED", "TRUE")
    assert hls_jobs.hls_enabled() is True


def test_hls_concurrency_parsing(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_HLS_CONCURRENCY", "3")
    assert hls_jobs.hls_concurrency() == 3
    monkeypatch.setenv("FUNERALACADEMY_HLS_CONCURRENCY", "0")
    assert hls_jobs.hls_concurrency() == 1  # clamped to >=1
    monkeypatch.setenv("FUNERALACADEMY_HLS_CONCURRENCY", "bad")
...
def test_hls_max_retries_parsing(monkeypatch):
    monkeypatch.delenv("FUNERALACADEMY_HLS_MAX_RETRIES", raising=False)
    assert hls_jobs.hls_max_retries() == 6
    monkeypatch.setenv("FUNERALACADEMY_HLS_MAX_RETRIES", "3")
    assert hls_jobs.hls_max_retries() == 3
    monkeypatch.setenv("FUNERALACADEMY_HLS_MAX_RETRIES", "bad")
    assert hls_jobs.hls_max_retries() == 6
