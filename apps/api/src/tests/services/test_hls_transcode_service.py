...
def test_build_ffmpeg_args_caps_threads(monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_HLS_FFMPEG_THREADS", "2")
...
def test_ffmpeg_threads_parsing(monkeypatch):
    monkeypatch.delenv("FUNERALACADEMY_HLS_FFMPEG_THREADS", raising=False)
    assert ht.ffmpeg_threads() == "1"  # default
    monkeypatch.setenv("FUNERALACADEMY_HLS_FFMPEG_THREADS", "3")
    assert ht.ffmpeg_threads() == "3"
    monkeypatch.setenv("FUNERALACADEMY_HLS_FFMPEG_THREADS", "bad")
...
def test_transcode_without_encryption(tmp_path, monkeypatch):
    monkeypatch.setenv("FUNERALACADEMY_HLS_ENCRYPT", "false")
    src = str(tmp_path / "src.mp4")
    out = str(tmp_path / "hls")
    _make_clip(src)
    result = asyncio.run(ht.transcode_source_to_hls(src, out))
