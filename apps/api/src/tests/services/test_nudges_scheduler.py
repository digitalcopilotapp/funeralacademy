...
class TestIdleLogging:
    ...
    def test_says_so_when_the_flag_is_unset(self, monkeypatch, caplog):
        ...
        assert "FUNERALACADEMY_NUDGES_ENABLED" in caplog.text
...
class TestStartGating:
    ...
    def _clean(self, monkeypatch):
        monkeypatch.delenv("FUNERALACADEMY_NUDGES_NO_SCHEDULER", raising=False)
        scheduler._task = None
        yield
        scheduler._task = None
    ...
    def test_can_be_opted_out_of(self, monkeypatch):
        ...
        monkeypatch.setenv("FUNERALACADEMY_NUDGES_NO_SCHEDULER", "1")
        scheduler.start_scheduler()
        assert scheduler._task is None
