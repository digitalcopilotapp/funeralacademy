...
When ``FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID`` is set, tokens minted for a
different OAuth client (``aud`` mismatch) must be rejected with 401 —
blocks the classic OAuth confused-deputy attack.

When neither ``FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID`` nor its
``FUNERALACADEMY_GOOGLE_CLIENT_ID`` alias is set we cannot check ``aud`` at
...
async def test_rejects_token_minted_for_different_oauth_client(monkeypatch):
    ...
    """
    monkeypatch.setenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", "our-client-id")
...
async def test_refuses_when_no_client_id_is_configured(monkeypatch):
    ...
    configure the check.
    """
    monkeypatch.delenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", raising=False)
    monkeypatch.delenv("FUNERALACADEMY_GOOGLE_CLIENT_ID", raising=False)
...
async def test_accepts_token_when_only_the_alias_env_var_is_set(monkeypatch):
    """The CLI env template emits FUNERALACADEMY_GOOGLE_CLIENT_ID; honour it."""
    monkeypatch.delenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", raising=False)
    monkeypatch.setenv("FUNERALACADEMY_GOOGLE_CLIENT_ID", "our-client-id")
