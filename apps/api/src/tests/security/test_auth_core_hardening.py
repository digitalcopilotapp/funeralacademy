...
GOOGLE_ENV_VARS = (
    "FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID",
    "FUNERALACADEMY_GOOGLE_CLIENT_ID",
)
...
class TestGoogleAudienceFailsClosed:
    ...
    async def test_blank_client_id_counts_as_unset(self, monkeypatch):
        monkeypatch.setattr(auth_utils, "_LOGGED_MISSING_GOOGLE_CLIENT_ID", False)
        monkeypatch.setenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", "   ")
    ...
    async def test_matching_audience_passes_for_either_spelling(
        ...
        FUNERALACADEMY_GOOGLE_CLIENT_ID; the API originally read only the _OAUTH_
    ...
    async def test_mismatched_audience_is_rejected(self, monkeypatch):
        monkeypatch.setenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", "ours")
        monkeypatch.delenv("FUNERALACADEMY_GOOGLE_CLIENT_ID", raising=False)
    ...
    async def test_expected_client_id_prefers_the_oauth_spelling(self, monkeypatch):
        monkeypatch.setenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID", "primary")
        monkeypatch.setenv("FUNERALACADEMY_GOOGLE_CLIENT_ID", "secondary")
        assert auth_utils.get_expected_google_client_id() == "primary"

        monkeypatch.delenv("FUNERALACADEMY_GOOGLE_OAUTH_CLIENT_ID")
