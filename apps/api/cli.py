...
async def _install_async(short: bool) -> None:
    funeralacademy_config = get_funeralacademy_config()
    sql_url = funeralacademy_config.database_config.sql_connection_string  # type: ignore
    ...
    try:
        async with AsyncSession(
            ...
            if short:
                ...
                # Honor FUNERALACADEMY_INITIAL_ORG_NAME / FUNERALACADEMY_INITIAL_ORG_SLUG when
                # the CLI passes them — falls back to "Default Organization" / "default"
                # so existing standalone deployments still work unchanged.
                org_name = os.environ.get("FUNERALACADEMY_INITIAL_ORG_NAME", "Default Organization")
                org_slug = os.environ.get("FUNERALACADEMY_INITIAL_ORG_SLUG", "default").lower()
                ...
                email = os.environ.get("FUNERALACADEMY_INITIAL_ADMIN_EMAIL", "admin@school.dev")
                # Require password from environment variable
                password = os.environ.get("FUNERALACADEMY_INITIAL_ADMIN_PASSWORD")
                if not password:
                    print("❌ Error: FUNERALACADEMY_INITIAL_ADMIN_PASSWORD environment variable is required")
                    print("Please set FUNERALACADEMY_INITIAL_ADMIN_PASSWORD environment variable before running installation.")
                    raise typer.Exit(code=1)
                print("Using password from FUNERALACADEMY_INITIAL_ADMIN_PASSWORD environment variable")
                if email != "admin@school.dev":
                    print(f"Using email from FUNERALACADEMY_INITIAL_ADMIN_EMAIL environment variable: {email}")
                user = UserCreate(
                ...
                print("email: " + email)
                print("password: (the password you set in FUNERALACADEMY_INITIAL_ADMIN_PASSWORD)")
...
def nudges_run(
    ...
    Cron-invoked. Sends nothing unless FUNERALACADEMY_NUDGES_ENABLED is set and the
...
def demo_sync():
    ...
    FUNERALACADEMY_DEMO_NO_SCHEDULER) or want to force a refresh now.
    """
