  ...
  return {
    ...
    execSync: vi.fn((cmd: string) => {
      if (typeof cmd === 'string' && cmd.includes('docker ps')) {
        return Buffer.from('funeralacademy-app-dep1\tUp 2 hours\tghcr.io/learnhouse/app:1.4.2\n')
      }
...
describe('doctor — DNS resolution check', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(installDir, '.env'), 'FUNERALACADEMY_DOMAIN=learn.example.com\n')
