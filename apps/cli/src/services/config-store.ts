...
import type { FuneralAcademyConfigJson, SetupConfig } from '../types.js'

export function writeConfig(config: SetupConfig): void {
  const data: FuneralAcademyConfigJson = {
    version: VERSION,
...
export function readConfig(dir?: string): FuneralAcademyConfigJson | null {
...
export function listInstallations(): { name: string; dir: string; config: FuneralAcademyConfigJson }[] {
  const baseDir = path.join(os.homedir(), '.funeralacademy')
  if (!fs.existsSync(baseDir)) return []

  const results: { name: string; dir: string; config: FuneralAcademyConfigJson }[] = []
  try {
...
export function findInstallDir(): string {
  ...
  if (installations.length > 1) {
    const running = installations.find((i) =>
      isContainerRunning(`funeralacademy-app-${i.config.deploymentId}`)
    )
