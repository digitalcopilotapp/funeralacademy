...
const API_ENV: AppEnvSpec = {
  ...
  vars: [
    {
      name: 'FUNERALACADEMY_AUTH_JWT_SECRET_KEY',
      required: true,
...
const WEB_ENV: AppEnvSpec = {
  ...
  vars: [
    {
      name: 'NEXT_PUBLIC_FUNERALACADEMY_BACKEND_URL',
      required: true,
...
const COLLAB_ENV: AppEnvSpec = {
  ...
  vars: [
    ...
    {
      name: 'FUNERALACADEMY_API_URL',
      required: true,
      description: 'FuneralAcademy API base URL',
      defaultValue: 'http://localhost:1338',
    },
    {
      name: 'FUNERALACADEMY_AUTH_JWT_SECRET_KEY',
...
export async function checkDevEnv(root: string): Promise<boolean> {
  ...
  const jwtSecret = apiExisting.get('FUNERALACADEMY_AUTH_JWT_SECRET_KEY') || generateJwtSecret()
  ...
  for (const app of ALL_APPS) {
    ...
    for (const v of app.vars) {
      ...
      if (v.name === 'FUNERALACADEMY_AUTH_JWT_SECRET_KEY') {
