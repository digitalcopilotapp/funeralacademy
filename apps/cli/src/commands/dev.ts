...
const PROJECT_NAME = 'funeralacademy-dev'

const DEV_COMPOSE = `name: funeralacademy-dev

services:
  db:
    image: pgvector/pgvector:pg16
    container_name: funeralacademy-db-dev
    restart: unless-stopped
    environment:
      - POSTGRES_USER=funeralacademy
      - POSTGRES_PASSWORD=funeralacademy
      - POSTGRES_DB=funeralacademy
    ports:
      - "5432:5432"
    volumes:
      - funeralacademy_db_dev_data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U funeralacademy"]
      interval: 5s
      timeout: 4s
      retries: 5
  ...
  redis:
    ...
    container_name: funeralacademy-redis-dev
    restart: unless-stopped
    command: redis-server --appendonly yes
    ports:
      - "6379:6379"
    volumes:
      - funeralacademy_redis_dev_data:/data
    healthcheck:
...
volumes:
  funeralacademy_db_dev_data:
  funeralacademy_redis_dev_data:
`
...
function getDevComposePath(root: string): string {
  const dotDir = path.join(root, '.funeralacademy')
...
function isInfraRunning(): boolean {
  return isContainerRunning('funeralacademy-db-dev') && isContainerRunning('funeralacademy-redis-dev')
}
...
export async function devCommand(opts: { ee?: boolean; adminEmail?: string; adminPassword?: string }) {
  ...
  if (!root) {
    p.log.error('Not inside a FuneralAcademy project.')
    p.log.info('Run this command from within the funeralacademy monorepo (must contain dev/docker-compose.yml, apps/api/, and apps/web/).')
    process.exit(1)
  }

  p.intro(pc.cyan('FuneralAcademy Dev Mode'))
  ...
  serviceEnv = {
    FORCE_COLOR: '1',
    FUNERALACADEMY_DEVELOPMENT_MODE: 'true',
    ...(adminEmail && { FUNERALACADEMY_INITIAL_ADMIN_EMAIL: adminEmail }),
    ...(adminPassword && { FUNERALACADEMY_INITIAL_ADMIN_PASSWORD: adminPassword }),
    ...(!opts.ee && { FUNERALACADEMY_DISABLE_EE: '1' }),
    // Bypass license verification for local dev when --ee is active
    ...(opts.ee && { FUNERALACADEMY_FORCE_EE: '1' }),
  }
  ...
  const [dbReady, redisReady] = await Promise.all([
    waitForHealth('DB', 'docker', ['exec', 'funeralacademy-db-dev', 'pg_isready', '-U', 'funeralacademy']),
    waitForHealth('Redis', 'docker', ['exec', 'funeralacademy-redis-dev', 'redis-cli', 'ping']),
  ])
  ...
  console.log()
  console.log(pc.dim('  Thank you for contributing to FuneralAcademy!'))
  console.log()
  ...
  const shutdown = async () => {
    ...
    console.log(pc.dim('To stop them: docker compose -f .funeralacademy/docker-compose.dev.yml -p funeralacademy-dev down'))
    console.log(pc.dim('Thanks for building with FuneralAcademy!'))
    process.exit(0)
  }
