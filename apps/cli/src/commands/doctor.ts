...
const REQUIRED_ENV_VARS = [
  'FUNERALACADEMY_DOMAIN',
  'FUNERALACADEMY_SQL_CONNECTION_STRING',
  'FUNERALACADEMY_REDIS_CONNECTION_STRING',
...
const EE_REQUIRED_ENV_VARS_BASE = [
  'FUNERALACADEMY_LICENSE_KEY',
  'ACME_EMAIL',
  'EE_IMAGE_TAG',
  'DB_PASSWORD',
  'FUNERALACADEMY_AUTH_JWT_SECRET_KEY',
  'COLLAB_INTERNAL_KEY',
  'FUNERALACADEMY_INITIAL_ADMIN_EMAIL',
  'FUNERALACADEMY_INITIAL_ADMIN_PASSWORD',
]
...
export async function doctorCommand() {
  ...
  p.intro(pc.cyan('FuneralAcademy Doctor'))
  ...
  if (!config) {
    p.log.warn('No FuneralAcademy installation found. Skipping deployment checks.')
    p.outro(pc.dim('Done'))
    return
  }
  ...
  if (containers.length === 0) {
    warn('No containers found', 'Run: npx funeralacademy start')
  } else {
    for (const c of containers) {
      ...
      if (isUp) {
        ...
        fail(`${svcName} is restarting`, 'Check logs: npx funeralacademy logs')
      } else {
        fail(`${svcName} — ${c.status}`, 'Run: npx funeralacademy start')
      }
    }
  }
  ...
  if (portFree) {
    ...
    if (hasRunning) {
      pass(`Port ${config.httpPort} in use (by FuneralAcademy services)`)
    } else {
  ...
  if (!fs.existsSync(envPath)) {
    fail('.env file missing', 'Run setup again: npx funeralacademy setup')
  } else {
