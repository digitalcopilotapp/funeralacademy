...
import type { FuneralAcademyConfigJson } from '../types.js'
...
export interface EditionLayout {
  appService: string      // compose service running the API (EE: 'api', CE: 'funeralacademy-app')
...
export function isExternalDbInstall(installDir: string): boolean {
  return !!readEnvVar(installDir, 'FUNERALACADEMY_SQL_CONNECTION_STRING')
}
...
export function backupDatabase(config: FuneralAcademyConfigJson, layout: EditionLayout, ui: UpdateLog): string {
  ...
  if (isExternalDbInstall(dir)) {
    const conn = readEnvVar(dir, 'FUNERALACADEMY_SQL_CONNECTION_STRING') || ''
    ...
    execSync(
      `docker compose exec -T ${layout.dbService} sh -c 'pg_dump -U "$\{POSTGRES_USER:-funeralacademy}" "$\{POSTGRES_DB:-funeralacademy}"' | gzip > ${JSON.stringify(out)}`,
...
export async function updateEnterprise(config: FuneralAcademyConfigJson, options: EeUpdateOptions): Promise<void> {
  const dir = config.installDir
  const ui: UpdateLog = {
  ...
  ui.log(`Upgrading FuneralAcademy Enterprise: ${pc.dim(currentTag)} → ${pc.cyan(targetTag)}`)
  ...
  const license = readEnvVar(dir, 'FUNERALACADEMY_LICENSE_KEY')
  if (!license) die('FUNERALACADEMY_LICENSE_KEY not found in .env — cannot pull EE images.')
  ...
  if (ready === 'oss') ui.warn('API came up in OSS mode — license not active. Check `funeralacademy logs`.')
  ...
  ui.ok(`FuneralAcademy Enterprise upgraded to ${targetTag}.`)
