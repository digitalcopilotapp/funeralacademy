...
async function stepInstallDir(): Promise<string | typeof BACK> {
  const baseDir = path.join(os.homedir(), '.funeralacademy')

  p.log.info(`All installations are stored in ${pc.cyan('~/.funeralacademy/<name>')}`)
  ...
  if (fs.existsSync(path.join(resolved, 'funeralacademy.config.json'))) {
    p.log.warn(`~/.funeralacademy/${name} already contains a FuneralAcademy installation.`)
...
export async function setupCommand(options: SetupOptions) {
  ...
  if (options.ci) {
    ...
    console.log(`Setting up FuneralAcademy in ~/.funeralacademy/${installName}`)
    ...
    if (options.start !== false) {
      ...
      try {
        ...
        if (healthy) {
          ...
          if (seeded) {
            console.log('FuneralAcademy is ready!')
          } else {
            console.error('')
            console.error(`API is healthy but organization "${config.orgSlug}" was not seeded.`)
            console.error('Run `npx funeralacademy logs` to inspect; pin a different image tag and re-run.')
            console.error('')
            process.exit(1)
          }
        } else {
  ...
  // ─── Interactive mode ───────────────────────────────────────
  await printBanner()
  p.intro(pc.cyan('FuneralAcademy Setup Wizard'))
  ...
  const s0 = p.spinner()
  s0.start('Resolving FuneralAcademy image version')
  ...
  const startNow = await p.confirm({
    message: 'Start FuneralAcademy now?',
    initialValue: true,
  })
  ...
  if (startNow) {
    p.log.step('Starting FuneralAcademy')
    const s2 = p.spinner()
    ...
    try {
      ...
      if (portAllocated) {
        ...
        p.log.info(
          'Fix one of these and re-run `funeralacademy start` (your config is already saved):\n' +
          `  • Stop whatever is using port ${config.httpPort}, or\n` +
          `  • Re-run \`funeralacademy setup\` and pick a different port, or\n` +
    ...
    // Health check
    const s3 = p.spinner()
    s3.start('Waiting for FuneralAcademy to be ready (up to 3 minutes)')
    ...
    if (healthy) {
      const seeded = await waitForOrgSeed(`http://localhost:${config.httpPort}`, config.orgSlug)
      if (seeded) {
        s3.stop('FuneralAcademy is ready!')
      } else {
        s3.stop(`API is healthy but organization "${config.orgSlug}" was not seeded`)
        p.log.error(
          `\`/api/v1/orgs/slug/${config.orgSlug}\` returns no org. Run \`funeralacademy logs\` to inspect, ` +
      ...
      p.log.warn('FuneralAcademy may still be starting. Check status with:')
      p.log.message(`  cd ${finalDir} && docker compose ps`)
    }

    // Success
    p.log.success(pc.green(pc.bold('FuneralAcademy is installed!')))
    p.log.message([
      '',
      ...
      `  ${pc.dim('$')} npx funeralacademy start    ${pc.dim('Start services')}`,
      `  ${pc.dim('$')} npx funeralacademy stop     ${pc.dim('Stop services')}`,
      `  ${pc.dim('$')} npx funeralacademy logs     ${pc.dim('View logs')}`,
      `  ${pc.dim('$')} npx funeralacademy config   ${pc.dim('Show configuration')}`,
      `  ${pc.dim('$')} npx funeralacademy backup   ${pc.dim('Backup & restore')}`,
      `  ${pc.dim('$')} npx funeralacademy deployments ${pc.dim('Manage deployments')}`,
      `  ${pc.dim('$')} npx funeralacademy doctor   ${pc.dim('Diagnose issues')}`,
      `  ${pc.dim('$')} npx funeralacademy shell    ${pc.dim('Container shell')}`,
      '',
    ].join('\n'))
  } else {
