...
async function createBackup() {
  ...
  const backupName = `funeralacademy-backup-${timestamp}`
  ...
  p.log.message([
    ...
    '',
    `  ${pc.dim('Restore with:')} npx funeralacademy backup --restore ${archivePath}`,
    '',
  ].join('\n'))
}
...
async function restoreBackup(archivePath: string) {
  ...
  p.log.info('You may want to restart services: npx funeralacademy stop && npx funeralacademy start')
}
...
export async function backupCommand(archivePath?: string, options?: { restore?: boolean }) {
  ...
  if (action === 'create') {
    ...
    const filePath = await p.text({
      ...
      placeholder: './backups/funeralacademy-backup-*.tar.gz',
    })
