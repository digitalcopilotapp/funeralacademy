...
import { getFUNERALACADEMY_DOMAIN_VAL, getFUNERALACADEMY_HTTP_PROTOCOL_VAL } from '@services/config/config'
...
function EmbedActivityClient({ activityId, courseuuid, orgslug, bgcolor }: EmbedActivityClientProps) {
  ...
  const showFuneralAcademyLogo = searchParams.get('showfuneralacademylogo') !== 'false'
  ...
  const getActivityUrl = () => {
    ...
    // (e.g. app.funeralacademy.io), so a relative path would resolve to the wrong host.
    if (typeof window !== 'undefined' && orgslug) {
      const domain = getFUNERALACADEMY_DOMAIN_VAL()
      const protocol = getFUNERALACADEMY_HTTP_PROTOCOL_VAL()
  ...
  if (!isEmbeddable) {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-8">
        ...
        {showFuneralAcademyLogo && <PoweredByBadge activityUrl={getActivityUrl()} />}
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen relative" style={customStyles}>
      ...
      {showFuneralAcademyLogo && ready && <PoweredByBadge activityUrl={getActivityUrl()} />}
    </div>
  )
}
