...
import { ImportTypeSelector, FuneralAcademyCourseImport } from '@components/Objects/Modals/Course/Import'
...
function CoursesHome(params: CourseProps) {
  ...
  const [importType, setImportType] = React.useState<'select' | 'scorm' | 'funeralacademy'>('select')
  ...
  const handleImportTypeSelect = (type: 'scorm' | 'funeralacademy') => {
    setImportType(type)
  }
  ...
  const getImportModalTitle = () => {
    switch (importType) {
      ...
      case 'funeralacademy':
        return t('dashboard.courses.import_funeralacademy')
      default:
  ...
  const getImportModalDescription = () => {
    switch (importType) {
      ...
      case 'funeralacademy':
        return t('dashboard.courses.import_funeralacademy_description')
      default:
  ...
  const bulkExportCourses = async () => {
    ...
    try {
      ...
      downloadBlob(blob, `funeralacademy-courses-export-${timestamp}.zip`)
