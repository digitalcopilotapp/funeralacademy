...
import type { FuneralAcademyPlanType } from "./plans";

export async function updateOrganizationConfigInternally(org_id: any, plan: FuneralAcademyPlanType) {
  ...
  const internalKey =
    process.env.CLOUD_INTERNAL_KEY || process.env.FUNERALACADEMY_CLOUD_INTERNAL_KEY || "";
  if (!internalKey) {
    throw new Error(
      "[updateOrgConfig] internal key unset — set CLOUD_INTERNAL_KEY (or " +
        "FUNERALACADEMY_CLOUD_INTERNAL_KEY) on the web deployment to match the API's " +
