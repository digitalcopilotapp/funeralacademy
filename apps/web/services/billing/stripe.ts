...
import { getServerAPIUrl, getFUNERALACADEMY_HTTP_PROTOCOL_VAL, getFUNERALACADEMY_DOMAIN_VAL } from "@services/config/config";
import {
  PRICE_IDS,
  PACK_PRICE_IDS,
  PRICE_TO_PLAN,
...
// but fall back to FUNERALACADEMY_STRIPE_SECRET_KEY (the platform Stripe account's
// secret key — the same account the SaaS subscription prices live in) so
// deployments that only set the FUNERALACADEMY_-prefixed var still work. Shared by
// the lazy Stripe client here, the webhook route, and the billing guard.
export function getStripeSecretKey(): string | undefined {
  return process.env.STRIPE_SECRET_KEY || process.env.FUNERALACADEMY_STRIPE_SECRET_KEY;
}
...
function appUrl(): string {
  return `${getFUNERALACADEMY_HTTP_PROTOCOL_VAL()}${getFUNERALACADEMY_DOMAIN_VAL()}`;
}
