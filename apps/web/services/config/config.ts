...
const getFUNERALACADEMY_HTTP_PROTOCOL = () =>
  (getConfig('NEXT_PUBLIC_FUNERALACADEMY_HTTPS') === 'true') ? 'https://' : 'http://'
const getFUNERALACADEMY_BACKEND_URL = () => getConfig('NEXT_PUBLIC_FUNERALACADEMY_BACKEND_URL', 'http://localhost/')
const getFUNERALACADEMY_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_FUNERALACADEMY_DOMAIN')
...
const getFUNERALACADEMY_TOP_DOMAIN = () => {
  // 1. Env var (backward compat for existing deploys)
  const envVal = getConfig('NEXT_PUBLIC_FUNERALACADEMY_TOP_DOMAIN')
  if (envVal) return envVal
  // 2. Cookie set by middleware from backend instance info
  const cookieVal = getCookieValue('LH_top_domain')
  if (cookieVal) return cookieVal
  // 3. Derive from DOMAIN by stripping port
  const domain = getFUNERALACADEMY_DOMAIN()
...
const getFUNERALACADEMY_PLATFORM_URL = (): string | null => {
  // NEXT_PUBLIC_ variant (available client-side via runtime config)
  const pubVal = getConfig('NEXT_PUBLIC_FUNERALACADEMY_PLATFORM_URL')
  if (pubVal) return pubVal.replace(/\/+$/, '')
  // Non-prefixed variant (server-side only, backward compat)
  const val = getConfig('FUNERALACADEMY_PLATFORM_URL')
  if (val) return val.replace(/\/+$/, '')
  return null
}

// Export getter functions for dynamic runtime configuration
export const getFUNERALACADEMY_HTTP_PROTOCOL_VAL = getFUNERALACADEMY_HTTP_PROTOCOL
export const getFUNERALACADEMY_BACKEND_URL_VAL = getFUNERALACADEMY_BACKEND_URL
export const getFUNERALACADEMY_DOMAIN_VAL = getFUNERALACADEMY_DOMAIN
export const getFUNERALACADEMY_TOP_DOMAIN_VAL = getFUNERALACADEMY_TOP_DOMAIN
export const getPOSTHOG_KEY_VAL = getPOSTHOG_KEY
export const getFUNERALACADEMY_PLATFORM_URL_VAL = getFUNERALACADEMY_PLATFORM_URL
...
export const FUNERALACADEMY_HTTP_PROTOCOL = getFUNERALACADEMY_HTTP_PROTOCOL()
export const FUNERALACADEMY_BACKEND_URL = getFUNERALACADEMY_BACKEND_URL()
export const FUNERALACADEMY_DOMAIN = getFUNERALACADEMY_DOMAIN()
export const FUNERALACADEMY_TOP_DOMAIN = getFUNERALACADEMY_TOP_DOMAIN()
...
// Derive API URL from backend URL (with backward compat for NEXT_PUBLIC_FUNERALACADEMY_API_URL)
const deriveAPIUrl = (): string => {
  // Backward compat: if explicit API URL is set, use it
  const explicitApiUrl = getConfig('NEXT_PUBLIC_FUNERALACADEMY_API_URL')
  if (explicitApiUrl) return explicitApiUrl
  // Derive from backend URL
  const backendUrl = getFUNERALACADEMY_BACKEND_URL().replace(/\/+$/, '')
...
export const getBackendUrl = () => getFUNERALACADEMY_BACKEND_URL()

/**
 * Get the upgrade/plan URL for a given org.
 *
 * In SaaS the billing/upgrade hub lives IN-APP on the apex (funeralacademy.io
 * /billing) — see app/(hub)/billing. We return an absolute apex URL so an
 * upgrade CTA rendered inside an org subdomain ({slug}.funeralacademy.io) crosses
...
/**
 * Build a URL on the platform domain (e.g. funeralacademy.app).
...
export const getPlatformUrl = (path: string): string | null => {
  const platformUrl = getFUNERALACADEMY_PLATFORM_URL()
...
//
// We deliberately do NOT consult `NEXT_PUBLIC_FUNERALACADEMY_MULTI_ORG` here —
...
export const getCustomDomainFromContext = (): string | null => {
  if (typeof window !== 'undefined') {
    ...
    // Check if current hostname is a custom domain (not a subdomain of FUNERALACADEMY_DOMAIN)
...
export const getUriWithOrg = (orgslug: string, path: string) => {
  ...
  if (typeof window !== 'undefined') {
    ...
    const baseDomain = stripPort(getFUNERALACADEMY_DOMAIN())
  ...
  if (orgslug) {
    const protocol = getFUNERALACADEMY_HTTP_PROTOCOL()
    const domain = getFUNERALACADEMY_DOMAIN()
    return `${protocol}${orgslug}.${domain}${path}`
  }
  const explicitDomain = getConfig('NEXT_PUBLIC_FUNERALACADEMY_DOMAIN')
  if (explicitDomain) {
    const protocol = getFUNERALACADEMY_HTTP_PROTOCOL()
...
export const getDefaultOrg = () => {
  ...
  const envVal = getConfig('NEXT_PUBLIC_FUNERALACADEMY_DEFAULT_ORG')
  if (envVal) return envVal
