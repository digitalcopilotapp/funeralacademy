...
import { FuneralAcademyEmail, type FuneralAcademyEmailProps } from '@components/Emails/FuneralAcademyEmail'
...
export async function send(
  to: string | string[],
  subject: string,
  props: FuneralAcademyEmailProps,
  ...
  try {
    const { data, error } = await client.emails.send({
      ...
      subject,
      react: React.createElement(FuneralAcademyEmail, props),
    })
    if (error) {
