...
// The configured org top-domain (e.g. "funeralacademy.io"), so we can allow safe
// redirects to org subdomains while still blocking external origins.
function topDomain(): string | null {
  const t =
    readCookie('LH_top_domain') ||
    getConfig('NEXT_PUBLIC_FUNERALACADEMY_TOP_DOMAIN') ||
    getConfig('NEXT_PUBLIC_FUNERALACADEMY_DOMAIN') ||
    ''
 ...
 * the configured org top-domain (e.g. *.funeralacademy.io). Blocks external origins,
