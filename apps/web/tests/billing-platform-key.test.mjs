...
beforeEach(() => {
  original = process.env.FUNERALACADEMY_PLATFORM_API_KEY;
});

afterEach(() => {
  if (original === undefined) delete process.env.FUNERALACADEMY_PLATFORM_API_KEY;
  else process.env.FUNERALACADEMY_PLATFORM_API_KEY = original;
});

describe("platformApiKey", () => {
  test("returns the configured key", () => {
    process.env.FUNERALACADEMY_PLATFORM_API_KEY = "platform-key";
    expect(platformApiKey()).toBe("platform-key");
  });

  test("throws when unset instead of returning an empty key", () => {
    delete process.env.FUNERALACADEMY_PLATFORM_API_KEY;
    expect(() => platformApiKey()).toThrow(/FUNERALACADEMY_PLATFORM_API_KEY is unset/);
  });

  test("treats an empty string as unset", () => {
    process.env.FUNERALACADEMY_PLATFORM_API_KEY = "";
    expect(() => platformApiKey()).toThrow(/FUNERALACADEMY_PLATFORM_API_KEY is unset/);
  });
});
