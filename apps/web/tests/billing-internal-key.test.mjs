...
const BOTH_NAMES = ["CLOUD_INTERNAL_KEY", "FUNERALACADEMY_CLOUD_INTERNAL_KEY"];
...
function setKeys({ cloud, prefixed }) {
  ...
  if (prefixed === undefined) delete process.env.FUNERALACADEMY_CLOUD_INTERNAL_KEY;
  else process.env.FUNERALACADEMY_CLOUD_INTERNAL_KEY = prefixed;
}
...
describe("updateOrganizationConfigInternally — internal key resolution", () => {
  ...
  test("sends the key when only FUNERALACADEMY_CLOUD_INTERNAL_KEY is set", async () => {
